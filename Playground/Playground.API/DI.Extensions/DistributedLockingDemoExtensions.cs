...
public static class DistributedLockingDemoExtensions
{
    ...
    // is a kind of connection management (like the concept connection pool) but instead of
    ...
    // the other end. 
    /// multiplexing has some same point with pipelining but in the redis sense, meaning that sending commands to the server without regard for the
    /// responsd being received => faster, it removes the latency between and awaiting the response to each item
    /// cons of multiplexing: because merging requests from multiple source into only one connections, low/heavy threads affect to other threads
