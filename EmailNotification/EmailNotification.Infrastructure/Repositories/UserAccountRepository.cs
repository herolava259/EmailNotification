...
namespace EmailNotification.Infrastructure.Repositories
{
    public class UserAccountRepository : BaseRepository<UserAccount>, IUserAccountRepository
    {
        ...
        public async Task<List<UserAccount>> GetUserAccountsNeedToBeChangedPassword(DateTimeOffset expiredDate)
        {
        ...
        public async Task<List<UserAccount>> GetUserOderedAccountsBefore(DateTimeOffset expiredDate)
        {
            var accountQuery = _dbContext.UserAccounts.AsNoTracking().OrderBy(c => c.Status)
                                                                     .OrderBy(c => c.CreatedDate)
                                                                    .Where(c => c.LastUpdatePassword < expiredDate.AddMonths(-6)
                                                                                && c.Status == EStatus.Normal);
            return await accountQuery.Include(c => c.Profile).ToListAsync();
        }
